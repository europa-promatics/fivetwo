	...
	constructor(private zone: NgZone, private authService: AuthService, private toastr: ToastrService) { }
  	...
  	ngAfterViewInit22() {
	  	this.zone.runOutsideAngular(() => {
			...
			lineSeries.tooltipText = "Holdings: {valueY.value}";
	  	...
	  	this.zone.runOutsideAngular(() => {
			...
			createSeries('first', ' 1 User Invest');
			createSeries('second', ' 2 User Invest');
			createSeries('third', ' 3 User Invest');
	...
	showMap() {
		...
		this.zone.runOutsideAngular(() => {
		  ...
		  function arrangeColumns() {
			  ...
			  if (series.dataItems.length > 1) 
			  {
				 chart.series.tooltipText = "{categoryX}: {valueY}";
				  var x0 = xAxis.getX(series.dataItems.getIndex(0), "categoryX");
				  var x1 = xAxis.getX(series.dataItems.getIndex(1), "categoryX");
				  var delta = ((x1 - x0) / chart.series.length) * w;
				  if (am4core.isNumber(delta)) {
