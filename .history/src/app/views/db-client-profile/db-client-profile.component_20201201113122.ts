...
export class DbClientProfileComponent implements OnInit {
  ...
  getInvestor(){
  	var investor = JSON.parse(sessionStorage.getItem('investor'))
    ...
    this.authService.clientProfile(ob).subscribe(data => {
  	...
  	if (investor!=null) {
  		this.investor=this.investor_data
  ...
  getTotalValue(){
    var obj = {
      client_id : this.investor_data.ClientNumber
    }
    this.authService.getTotalValue().subscribe(data => {
      alert(data);
        if(data.success){
