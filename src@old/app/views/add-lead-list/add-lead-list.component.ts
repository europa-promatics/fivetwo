...
export class AddLeadListComponent implements OnInit {
  ...
  placeChangedCallback(evt:any){
   ...
   form = new FormGroup({
        ...
        IdNumber: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-Z0-9 ]*'),Validators.maxLength(15)]),
        CellNumber: new FormControl('', [Validators.required, Validators.pattern('^[0-9]*'),Validators.maxLength(15)]),
        Email: new FormControl('', [Validators.required,Validators.pattern('^[a-zA-Z0-9]+(\.[_a-z0-9]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,15})$')]),
        HomeAddress: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-Z0-9 #,.]*'),Validators.maxLength(255)]),
